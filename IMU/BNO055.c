...
* Modified on July 26, 2022 5:29 pm for Raspberry pi
*/
...
#include <time.h> //has nanosleep function and gettime
...
char BNO055_Init(int i2c_handle) {
    ...
    byteReturn = i2cReadByteData(i2c_handle, BNO055_CHIP_ID_ADDR);
    if (byteReturn != BNO055_ID) {
        printf("IMU not found! \r\n");
        return (-1);
    } else {
        printf("Device with address %x found\r\n", byteReturn);
    }
    ...
    byteReturn = i2cWriteByteData(i2c_handle,BNO055_OPR_MODE_ADDR, OPERATION_MODE_CONFIG);
    usleep(250000);  // delay between changing op modes > 19 msec
    /*set the register page to page 1*/
    byteReturn = i2cWriteByteData(i2c_handle, BNO055_PAGE_ID_ADDR, BNO055_PAGE1);
    usleep(2000);
    /* Config gyro for 250 dps */
    byteReturn = i2cWriteByteData(i2c_handle, BNO055_GYR_CONFIG_0, GYRO_CONFIG_PARAMS_0);
    usleep(2000);
    /* Config accelerometer to +/- 2g */
    byteReturn = i2cWriteByteData(i2c_handle, BNO055_ACC_CONFIG, ACC_CONFIG_PARAMS);
    usleep(2000);
    /*set the register page to page 0*/
    byteReturn = i2cWriteByteData(i2c_handle, BNO055_PAGE_ID_ADDR, BNO055_PAGE0);
    usleep(2000);
    // /*set axis to P0*/
    // byteReturn = I2C_WriteReg(BNO055_ADDRESS_A,BNO055_AXIS_MAP_CONFIG_ADDR, P1_AXIS);
    // /*set sign to P0*/
    // byteReturn = I2C_WriteReg(BNO055_ADDRESS_A,BNO055_AXIS_MAP_SIGN_ADDR, P1_SIGN);
    /*set units*/
    byteReturn = i2cWriteByteData(i2c_handle, BNO055_UNIT_SEL_ADDR, UNITS_PARAM);
    usleep(2000);
    /* Set operation mode to AMG */
    byteReturn = i2cWriteByteData(i2c_handle, BNO055_OPR_MODE_ADDR, OPERATION_MODE_AMG);
    usleep(200000);
    return (byteReturn);
}
/**
...
int BNO055_ReadAccel(int i2c_handle, int16_t data[]){
    data[0] = i2cReadWordData(i2c_handle, BNO055_ACCEL_DATA_X_LSB_ADDR);
    data[1] = i2cReadWordData(i2c_handle, BNO055_ACCEL_DATA_Y_LSB_ADDR);
    data[2] = i2cReadWordData(i2c_handle, BNO055_ACCEL_DATA_Z_LSB_ADDR);
    return(0);
}
/**
 * @Function BNO055_Read*()
...
int BNO055_ReadMag(int i2c_handle, int16_t data[]){
    data[0] = i2cReadWordData(i2c_handle, BNO055_MAG_DATA_X_LSB_ADDR);
    data[1] = i2cReadWordData(i2c_handle, BNO055_MAG_DATA_Y_LSB_ADDR);
    data[2] = i2cReadWordData(i2c_handle, BNO055_MAG_DATA_Z_LSB_ADDR);
    return(0);
}
/**
 * @Function BNO055_Read*()
...
int BNO055_ReadGyro(int i2c_handle, int16_t data[]){
    data[0] = i2cReadWordData(i2c_handle, BNO055_GYRO_DATA_X_LSB_ADDR);
    data[1] = i2cReadWordData(i2c_handle, BNO055_GYRO_DATA_Y_LSB_ADDR);
    data[2] = i2cReadWordData(i2c_handle, BNO055_GYRO_DATA_Z_LSB_ADDR);
    return(0);
}



// #define BNO055_TESTING
#ifdef BNO055_TESTING
...
int main(void){
    ...
    long int timeout = 20; // timeout in sec
    char timer_active = 1;
    int16_t acc[3] = {0,0,0};
    int16_t mag[3] = {0,0,0};
    ...
    init_return = BNO055_Init(i2c_handle);
    ...
    while(timer_active){
        ...
        elapsed_time = (current_time.tv_sec - start_time.tv_sec)*sec2nsec + (current_time.tv_nsec - start_time.tv_nsec);
        if(elapsed_time >= period){
            /*reset start time*/
            clock_gettime(CLOCK_REALTIME,&start_time);
            BNO055_ReadAccel(i2c_handle, acc);
            BNO055_ReadMag(i2c_handle, mag);
            BNO055_ReadGyro(i2c_handle, gyro);
            /*print data to console*/
            printf("%d, %d, %d, ", acc[0],acc[1],acc[2]);
            printf("%d, %d, %d, ", mag[0],mag[1],mag[2]);
            printf("%d, %d, %d, %ld\r\n", gyro[0],gyro[1],gyro[2], elapsed_time);
        }/*end if*/
        /* check for timeout*/
